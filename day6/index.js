...
among the banks. To do this, it removes all of the blocks from the selected bank, then 
moves to the next (by index) memory bank and inserts one of the blocks. It continues 
doing this until it runs out of blocks; if it reaches the last memory bank, it wraps 
...
At this point, we've reached a state we've seen before: 2 4 1 2 was already seen. The
 infinite loop is detected after the fifth block redistribution cycle, and so the answer in this example is 5.

Given the initial block counts in your puzzle input, how many redistribution cycles must 
be completed before a configuration is produced that has been seen before?
...
/**
 * arrayMatch
 * checks to see if sequence matches a previous array
 * @param {Array[Number]} prev 
 * @returns {Boolean}
 */
function arraysMatch(prev) {
    let answer = true;
...
/**
 * seenBefore
 * checks previous state to sequence for a match
 * @returns {Boolean}
 */
function seenBefore() {
    let answer = false;
    for(let i = 0; i < state.length; i++) {
        if (arraysMatch(state[i])) {
            answer = true;
            matchIndex = i;
            break;
        }
 ...
 * if multiple it matches first instance
 * @returns {Number}
 */
...
/**
 * addToState
 * clones current sequence and adds to state
 * @returns {void}
 */
function addToState() {
    let newSeq = [];
...
/**
 * balance
 * takes value of the index of largest number
 * and distributes it across all indexes
 * @param {Number} index  
 * @returns {void}
 */
...
while(!seenBefore()) {
    addToState();
    balance(findLargestIndex());
}

console.log(`It took ${state.length} cycles to duplicate state which was ${state.length - matchIndex} cycles ago`);