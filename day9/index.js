...
You sit for a while and record part of the stream (your puzzle 
  ...
  other things, separated by commas: either another group or garbage. 
  Since groups can contain other groups, a } only closes the 
...
function countGroups (str) {
  ...
  for(let i = 0; i < str.length; i++) {
    let character = str[i];
    if(character === '!') i++;  // ignore next character
    else if (garbage && character !== '>') garbageCount++; // everything until next > is garbage
    else if (character === '<') garbage = true; // starts a garbage section
    else if (character === '>') garbage = false; // ends a garbage section
    else if (character === '{') total += depth++; // count opening of group based on depth
    else if (character === '}') depth--; // group is closed, decrease depth
    // comma between groups that are not in garbage are ignored.
  }
  return {total, garbageCount};
}


let countedGroups = countGroups(input);
console.log(`The total for the groups is ${countedGroups.total} and total garbage is ${countedGroups.garbageCount}`);
